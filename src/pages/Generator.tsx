...
const Generator: React.FC = () => {
  ...
  const handleGenerate = (e: React.FormEvent) => {
    ...
    const serverName = `${selectedProxy.country} - ${selectedProxy.provider} [${activeTab.toUpperCase()}-${formSecurity === 'tls' ? 'TLS' : 'NTLS'}]`;
        ...
        <div className={`lg:col-span-5 flex flex-col lg:h-full lg:min-h-0 transition-all duration-300 ${mobileView === 'detail' ? 'hidden lg:flex' : 'flex'}`}>
          <div className="gento-card rounded-3xl flex flex-col h-full overflow-hidden">
            ...
            <div className="relative flex-grow flex flex-col overflow-hidden bg-slate-900/20">
                <div className="flex-grow overflow-y-auto p-3 space-y-2 custom-scroll">
                ...
                {isLoadingList && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-900/80 z-10 backdrop-blur-sm">
                        <i className="fas fa-circle-notch fa-spin text-2xl text-purple-500 mb-2"></i>
                        <span className="text-xs text-slate-400">Memuat List...</span>
                    </div>
                )}
            </div>
        ...
        <div className={`lg:col-span-7 flex flex-col lg:h-full lg:min-h-0 animate-fade-in-up transition-all duration-300 ${mobileView === 'list' ? 'hidden lg:flex' : 'flex'}`}> 
             ...
             {!showResult ? (
                 <div className="gento-card rounded-3xl p-6 flex flex-col flex-grow overflow-y-auto">
                     ...
                     <form onSubmit={handleGenerate} className="space-y-4 flex-grow flex flex-col">
                         <div className="space-y-1">
                             ...
                             <input 
                               type="text" 
                               value={selectedProxy ? `${selectedProxy.country} - ${selectedProxy.provider} [${activeTab.toUpperCase()}-${formSecurity==='tls'?'TLS':'NTLS'}]` : ''} 
                               readOnly 
                         ...
                         <div className="space-y-1">
                             <label className="text-[10px] font-bold text-slate-500 uppercase ml-1">Bug / SNI</label>
                 ...
                 <div className="gento-card rounded-3xl p-6 flex flex-col flex-grow overflow-y-auto animate-fade-in">
                     ...
                     <div className="space-y-4 pr-1 custom-scroll">
                         <div className="bg-white p-4 rounded-2xl flex justify-center w-max mx-auto shadow-xl">
                             <div ref={qrRef}></div>
                         </div>
                         <div className="space-y-2">
                             <div className="flex justify-between items-center"><span className="text-[10px] font-bold text-slate-500 uppercase">Connection URL</span><button onClick={() => { copyToClipboard(resultUrl); showToast('Copied!'); }} className="text-xs text-purple-400 hover:text-white"><i className="far fa-copy"></i> Copy</button></div>
                         ...
                         <div className="space-y-2">
                             ...
                             <div className="gento-input rounded-xl p-3">
                                 <pre className="font-mono text-[10px] text-emerald-300 overflow-x-auto whitespace-pre max-h-32 custom-scroll"><code>{resultClash}</code></pre>
                             </div>
                         </div>
                     </div>
                     <div className="grid grid-cols-2 gap-3 mt-4 pt-4 border-t border-white/5 flex-shrink-0">
                         <button onClick={() => { copyToClipboard(`URL:\n${resultUrl}\n\nCLASH:\n${resultClash}`); showToast('All Copied!'); }} className="py-3.5 rounded-xl bg-slate-800 text-white text-xs font-bold hover:bg-slate-700 border border-white/5 transition-colors">COPY ALL</button>
                         <button onClick={() => { setFormUUID(generateUUID()); setFormPassword(generateUUID()); setShowResult(false); }} className="py-3.5 rounded-xl bg-purple-600 text-white text-xs font-bold hover:bg-purple-500 shadow-lg transition-colors">CREATE NEW</button>
                     </div>
                 </div>
             )}
        </div>
